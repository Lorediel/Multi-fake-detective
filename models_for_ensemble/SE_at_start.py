...
import random
from utils.utils import format_metrics, compute_metrics
import os
...
class SEText_0(nn.Module):
    def __init__(self):
        super(SEText_0, self).__init__()
        self.fc1 = nn.Sequential(
            nn.Linear(3,3),
            nn.GELU(),
        )
    ...
    def forward(self, bert, sentiment, clip):
        bert_mean = torch.mean(bert, dim = 1).unsqueeze(1) # batch size x 1
        sentiment_mean = torch.mean(sentiment, dim = 1).unsqueeze(1)  # batch size x 1
        clip_mean = torch.mean(clip, dim = 1).unsqueeze(1)  # batch size x 1
        avg = torch.cat((bert_mean, sentiment_mean, clip_mean), dim = 1) # batch size x 3
        y = self.fc1(avg) # batch size x 3 
        y = self.fc2(y) # batch size x 3 
        y_bert = y[:,0].unsqueeze(1)
        y_sentiment = y[:,1].unsqueeze(1)
        y_clip = y[:,2].unsqueeze(1)
        return bert * y_bert, sentiment * y_sentiment, clip * y_clip
    
class SEVisual_0(nn.Module):
    def __init__(self):
        super(SEVisual_0, self).__init__()
        self.fc1 = nn.Sequential(
            nn.Linear(2,2),
            nn.ReLU(),
        )
    ...
    def forward(self, resnet, clip_v):
        resnet_mean = torch.mean(resnet, dim = 1).unsqueeze(1) # batch size x 1
        clip_v_mean = torch.mean(clip_v, dim = 1).unsqueeze(1) # batch size x 1
        avg = torch.cat((resnet_mean, clip_v_mean), dim = 1) # batch size x 2
        y = self.fc1(avg) # batch size x 2
        y = self.fc2(y) # batch size x 2
        y_r = y[:,0].unsqueeze(1)
        y_c = y[:,1].unsqueeze(1)
        return resnet * y_r, clip_v * y_c
...
class FNDModel(nn.Module):
    def __init__(self, bert_model, resnet_model, clip_model, norm_weight = 0.5):
        ...
        self.setext0 = SEText_0()
        self.sevisual0 = SEVisual_0()
    ...
    def forward(self, texts, images):
        ...
        bert_embeddings, sentiment_embeddings, clip_text_embeddings = self.setext0(bert_embeddings, sentiment_embeddings, clip_text_embeddings)
        #clip_text_embeddings, clip_image_embeddings = self.seclip0(clip_text_embeddings, clip_image_embeddings)
        resnet_embeddings, clip_image_embeddings = self.sevisual0(resnet_embeddings, clip_image_embeddings)
        ...
        concatenated_embeddings = torch.cat([textual_embeddings, visual_embeddings, clip_embeddings], dim=1)
        embeds = self.squeeze(concatenated_embeddings)

        final_embeds = torch.sum(embeds, dim=1) # batch size x 1024
        logits = self.classifier(final_embeds)
        return logits
...
class FNDTrainer:
    ...
    def train(self, train_ds, eval_ds, lr = 1e-3, batch_size= 8, num_epochs = 30, warmup_steps = 0, save_path = "./", loss= "cross_entropy", weight_decay=0.001):
