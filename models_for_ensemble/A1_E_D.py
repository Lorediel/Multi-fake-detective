from transformers import AutoModel, AutoTokenizer, ResNetModel, AutoImageProcessor, VisionTextDualEncoderModel, AutoProcessor, AdamW, get_scheduler
import torch
...
class FNDModel(nn.Module):
    def __init__(self, bert_model, resnet_model, clip_model, norm_weight = 0.5):
        ...
        self.FFTModule = FFTModule(self.device)
        ...
        self.fft_projection_head = nn.Sequential(
            nn.Linear(1024, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Dropout(0.2),
    ...
    def forward(self, texts, images):
        ...
        fft_embeddings = self.FFTModule(images)
        ...
        sentiment_embeddings = self.sentiment_projection_head(sentiment_embeddings).unsqueeze(1)
        fft_embeddings = self.fft_projection_head(fft_embeddings).unsqueeze(1)

        clip_embeddings = (clip_embeddings * normalized_clip_similarity).unsqueeze(1)

        #concatenate the features 
        concatenated_embeddings = torch.cat([textual_embeddings, sentiment_embeddings, visual_embeddings, clip_embeddings, fft_embeddings], dim=1)
...
class FNDTrainer:
    ...
    def train(self, train_ds, eval_ds, lr = 1e-3, batch_size= 8, num_epochs = 30, warmup_steps = 0, save_path = "./", loss= "cross_entropy"):
...
class VGGModel(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device
        backbone = torch.hub.load('pytorch/vision:v0.10.0', 'vgg19', pretrained=True).to(self.device)
    ...
    def forward(self, images):
        ...
        pooled_features = self.pooling(features).squeeze() # batch_size x 512 x 1 x 1 -> batch_size x 512
...
class FFTModule(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device
        self.vggReal = VGGModel(device)
        self.vggImag = VGGModel(device)
    ...
    def forward(self, images):
        ...
        imag_parts = []
        for img in images:
            fft2 = np.fft.fft2(np.array(img).transpose(2, 0, 1))
            real_part = torch.tensor(np.real(fft2)).to(self.device)
            imag_part = torch.tensor(np.imag(fft2)).to(self.device)

            real_part_pp = self.preprocess(real_part)
            imag_part_pp = self.preprocess(imag_part)
            real_parts.append(real_part_pp)
            imag_parts.append(imag_part_pp)
        real_parts = torch.stack(real_parts).float()
        imag_parts = torch.stack(imag_parts).float()
        real_features = self.vggReal(real_parts)
        imag_features = self.vggImag(imag_parts)
        return torch.cat((real_features, imag_features), dim = -1)
